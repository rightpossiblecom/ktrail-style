import type { Metadata } from 'next';
import Link from 'next/link';
import { siteConfig } from '@/config/site';

export const metadata: Metadata = {
	title: 'Features',
	description: siteConfig.featuresPage.body,
};

export default function FeaturesPage() {
	const { featuresPage, features, howItWorks, hero } = siteConfig;

	return (
		<main className="bg-[var(--color-bg)] text-[var(--color-cream)]">
			<section className="mx-auto max-w-[1500px] px-6 pt-32 pb-16 lg:px-10 lg:pt-40">
				<p className="text-[0.65rem] font-medium uppercase tracking-[0.32em] text-[var(--color-copper)]">
					{featuresPage.eyebrow}
				</p>
				<h1 className="heading-display mt-6 text-5xl leading-[0.95] lg:text-7xl">
					<span className="block">{featuresPage.heading[0]}</span>
					<span className="block">
						{featuresPage.heading[1]}{' '}
						<span className="heading-display-italic text-[var(--color-copper)]">
							{featuresPage.headingAccent}
						</span>
					</span>
				</h1>
				<p className="mt-8 max-w-2xl text-base leading-relaxed text-[var(--color-cream-muted)]">
					{featuresPage.body}
				</p>
			</section>

			<section className="border-t border-[var(--color-line)] py-16">
				<div className="mx-auto max-w-[1500px] px-6 lg:px-10">
					<ol className="divide-y divide-[var(--color-line)] border-y border-[var(--color-line)]">
						{features.map((feature, i) => (
							<li key={feature.title} className="grid gap-4 py-10 lg:grid-cols-12 lg:items-start">
								<span className="heading-display text-2xl text-[var(--color-copper)] lg:col-span-2">
									{String(i + 1).padStart(2, '0')}
								</span>
								<h2 className="heading-display text-2xl text-[var(--color-cream)] lg:col-span-4">
									{feature.title}
								</h2>
								<p className="text-sm leading-relaxed text-[var(--color-cream-muted)] lg:col-span-6">
									{feature.body}
								</p>
							</li>
						))}
					</ol>
				</div>
			</section>

			<section className="border-t border-[var(--color-line)] bg-[var(--color-bg-deep)] py-20">
				<div className="mx-auto max-w-[1500px] px-6 lg:px-10">
					<p className="text-[0.65rem] font-medium uppercase tracking-[0.32em] text-[var(--color-copper)]">
						How it works
					</p>
					<div className="mt-10 grid gap-8 lg:grid-cols-3">
						{howItWorks.map((step) => (
							<div key={step.step}>
								<span className="heading-display text-2xl text-[var(--color-copper)]">{step.step}</span>
								<h3 className="heading-display mt-4 text-xl text-[var(--color-cream)]">{step.title}</h3>
								<p className="mt-3 text-sm text-[var(--color-cream-muted)]">{step.body}</p>
							</div>
						))}
					</div>
					<div className="mt-12 flex flex-wrap gap-4">
						<Link
							href={hero.cta.href}
							className="rounded-full bg-[var(--color-copper)] px-8 py-4 text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--color-bg-deep)]"
						>
							{hero.cta.label}
						</Link>
						<Link
							href={hero.secondaryCta.href}
							className="link-underline text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--color-cream)]"
						>
							{hero.secondaryCta.label}
						</Link>
					</div>
				</div>
			</section>
		</main>
	);
}
